...
import AboutImage from "/src/assets/aboutus.jpg";
...
const Home = () => {
  ...
  const products = [
    { id: 1, name: "Koneksyon T-Shirt", category: "T-Shirts", image: "/src/assets/koneksyon1.jpg" },
    { id: 2, name: "Hard Core Hoodie", category: "Hoodies", image: "/src/assets/jacket2.jpg" },
    { id: 3, name: "Rapollo Cap", category: "Caps", image: "/src/assets/cap.jpg" },
    { id: 4, name: "Atraxia bag", category: "Accessories", image: "/src/assets/accessory.jpg" },
  ];
  ...
  return (
    <div className="home-container w-full min-h-screen bg-black text-white font-hanuman">
      ...
      <section
        className="relative w-full h-[80vh] flex flex-col justify-center items-center bg-cover bg-center"
        style={{backgroundImage: "url('/src/assets/rapollohomegif.gif')" }}
      >
        {/* <div className="absolute inset-0 bg-black/50"></div>
        <img src={Logo} alt="Logo" className="w-90% md:w-90% relative z-50" /> */}
      </section>

      <section className="relative w-full h-[80vh] flex flex-col justify-center items-center bg-cover bg-center">
        <div 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full px-6 mt-6">
          {events.length > 0 ? (
            events.map((event) => (
              <motion.div
                key={event.id}
                className="bg-gray-800 border border-gray-700 rounded-lg p-4 hover:scale-105 transition-transform"
      ...
      <section className="w-full py-16 bg-black text-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full px-6 mt-6">
          {products.slice(0, 4).map((product) => (
            <motion.div
              key={product.id}
              className="bg-gray-900 p-4 rounded-lg border border-gray-700 shadow-lg cursor-pointer hover:scale-105 transition-transform"
              whileHover={{ scale: 1.05 }}
            >
